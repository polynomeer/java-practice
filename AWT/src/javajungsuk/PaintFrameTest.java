...
class PaintFrameTest {
    public static void main(String args[]) {
        PaintFrame f = new PaintFrame("javajungsuk.PaintFrame");
        Graphics g = f.getGraphics();
        g.setColor(Color.red);
        for(int i=0; true ; i+=f.speed) {
            try {
                Thread.sleep(100); // 0.1 . 초 동안 실행을 지연시킨다
...
class PaintFrame extends Frame {
    ...
    PaintFrame(String title) {
        ...
        setLayout(null);
        setVisible(true);
        setResizable(false); // Frame . 의 크기를 변경할 수 없도록 한다
    }
    class MyHandler extends WindowAdapter implements AdjustmentListener {
        public void adjustmentValueChanged(AdjustmentEvent e){
            speed = sb.getValue(); // Scrollbar . 의 현재 값을 얻는다
